		...
		const fetchNotifications = async () => {
			try {
				...
				if (data) {
					...
					const notificationList = data.map(
						(notification: APINotification) => {
							...
							if (notification.sender_info) {
								ret.SenderInfo = {
									...
									UserName:
										notification.sender_info.user_name,
									ProfileImageUrl:
										notification.sender_info
											.profile_image_url,
									IsPrivate:
										notification.sender_info.is_private,
							...
							if (notification.related_tweet) {
								ret.RelatedTweet = {
									TweetId:
										notification.related_tweet.tweet_id,
									UserInfo: {
										UserId: notification.related_tweet
											.user_info.user_id,
										UserName:
									...
									RetweetsCount:
										notification.related_tweet
											.retweets_count,
									RepliesCount:
										notification.related_tweet
											.replies_count,
									IsQuote:
										notification.related_tweet.is_quote,
									IsReply:
										notification.related_tweet.is_reply,
									IsPinned:
										notification.related_tweet.is_pinned,
									HasLiked:
										notification.related_tweet.has_liked,
									HasRetweeted:
										notification.related_tweet
											.has_retweeted,
									CreatedAt:
								...
								if (notification.related_tweet.media) {
									ret.RelatedTweet.Media = {
										type: notification.related_tweet.media
											.type,
										url: notification.related_tweet.media
											.url,
									} as Media;
								}
							}
